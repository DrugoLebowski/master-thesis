...
\begin{algorithm}
	\begin{algorithmic}[1]
		\Function{RunCircuit}{Conf, Gates, MaxInt, IMem, Regs}
			...
			\State{Distributions = []}
			\newline
			\ForEach{$g\in \textrm{Gates}$}\Comment{Run of the circuit}
				...
				\ElsIf{\Call{g.isUnary}}
					...
					\State{InputValue = \Call{Avg}{RegsAndOutput, Coeff}}
				...
				\Else
					...
					\State{InputValueA = \Call{Avg}{RegsAndOutput, CoeffA}}
					\State{InputValueB = \Call{Avg}{RegsAndOutput, CoeffB}}
					\State{GateResult = g.\Call{execute}{GateValueA, GateValueB, IMem, MaxInt}}	
				\EndIf
				\State{RegsAndOutput = \Call{Concatenate}{Regs, GateValue}}
			\EndFor
			\newline
			\For{$i\in[1, \dots, |Regs|]$}\Comment{Update of the registers}
				\State{Coeff = \Call{GetGateCoefficient}{Conf, i}}
				\State{NewValue = \Call{Avg}{RegsAndOutput, Coeff}}
