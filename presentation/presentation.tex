  ...
  \begin{frame}{Mutation variants}
  	...
  	\begin{itemize}
  		...
  		\uncover<5->{\item{\textbf{Current-to-pbest}: exists in two versions:
			\begin{itemize}
				\uncover<6->{\item{without memory: $\textbf{u}_{i, G} = \textbf{x}_{i, G} + F_i \cdot (\textbf{x}_{\textit{best}, G}^p - \textbf{x}_{i, G}) + F_i \cdot (\textbf{x}_{r_1, G} - \textbf{x}_{r_2, G})$}}
				\uncover<7->{\item{with memory: $\textbf{u}_{i, G} = \textbf{x}_{i, G} + F_i \cdot (\textbf{x}_{\textit{best}, G}^{p} - \textbf{x}_{i, G}) + F_i \cdot (\textbf{x}_{r_1, G} - \tilde{\textbf{x}}_{r_2, G})$}}
			\end{itemize}			  		
  		}}
	\end{itemize}  	 
  \end{frame}
